  ...
  public  class GxBitmap
    {
      ...
      public void SaveBmp(IBaseData objIBaseData, string strFilePath)
      {
          ...
          if (null != objIBaseData)
          {
              ...
              string file_path = strFilePath;//string.Format("CamFile{0}", strFilePath);

                var sw = new System.IO.StreamWriter("SaveBmp.txt", true);
                sw.WriteLine(DateTime.Now.ToString() + ":" + "SaveBmp1111:" + "" + file_path + "\r\n" + "");
                sw.Close();

                m_bitmapForSave.Save(file_path, ImageFormat.Jpeg);
            }
      }

        public ImageFormat ReturnBmp(IBaseData objIBaseData, string strFilePath)
        {
            GX_VALID_BIT_LIST emValidBits = GX_VALID_BIT_LIST.GX_BIT_0_7;
